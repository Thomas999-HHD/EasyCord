...
## [3.9.0] — 2026-04-26 (Foundation Release)

**Foundation release establishing structural groundwork for v4.0 modular architecture.**

Reduces lock-in perception, introduces EventBus and CapabilityRegistry, maintains full v3.8 compatibility.

### ✨ Added

- **Public API Namespace** — `easycord.api.v1` (stable, frozen for SemVer through v4.x)
- **Escape Hatches** — `ctx.raw_interaction`, `ctx.client` (direct discord.py access, always)
- **EventBus** — `bot.events` (async pub/sub with priority, wildcards, concurrent handlers)
- **CapabilityRegistry** — `bot.capability_registry` (permission gates for moderation, AI, custom actions)
- **Middleware Alias** — `@bot.middleware` (v4.0 naming, `@bot.use()` still works)
- **Documentation** — EXECUTION_PIPELINE.md, PLUGIN_V2_SPEC.md, MIGRATION.md

### 🔄 Changed

- Discord events now emit through EventBus (legacy v3 handlers still work)
- Slash commands accept optional `capabilities=` parameter
- Module structure: added `easycord/kernel/` and `easycord/api/v1/`

### ⚠️ Deprecations

- `from easycord import ...` still works, recommend `from easycord.api.v1 import ...`

### 🧪 Experimental

- EventBus (production-ready, v4.0 expands patterns)
- CapabilityRegistry (production-ready, v4.0 expands plugin integration)

### 📋 Upcoming (v4.0)

- Plugin v2 system (explicit lifecycle, isolation, hot reload)
- TypedStore (per-guild config, atomic writes, migrations)
- Storage backend abstraction (Postgres, Redis)
- AI decoupling (`easycord[ai]` optional extra)
- CLI tools (`easycord init`, `easycord dev`)

### 🤝 Backwards Compatibility

✅ **100% compatible with v3.8.** Zero breaking changes. All v3.8 code runs unchanged.

### 📖 Migration

See [docs/MIGRATION.md](docs/MIGRATION.md) for adoption patterns.
