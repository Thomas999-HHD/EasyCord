...
Stable release: Bug fixes, architecture cleanup, 691 tests passing.

### Fixed
- **asyncio deprecation** — All AI provider `query()` methods now use `asyncio.get_running_loop()` instead of deprecated `asyncio.get_event_loop()`
- **Bulk moderation logging** — `bulk_timeout`, `bulk_role_add`, `bulk_role_remove` now log failures at WARNING level
- **Test suite** — 14 test fixes for i18n sub-package mock paths, provider SDK changes, plugin reload semantics

### Changed
- **Removed dead `rate_limit` parameter from `@slash`** — was stored but never enforced; use `cooldown` instead
- **i18n split** — `easycord/i18n.py` refactored into `easycord/i18n/` sub-package (public API unchanged)
- **Plugin command registry** — slash commands tracked in `bot.registry.commands` with name, description, plugin, and guild scope
