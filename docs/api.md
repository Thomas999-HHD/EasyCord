...
`Bot.slash(name=None, *, description, guild_id=None, guild_only=False, ephemeral=False, permissions=None, cooldown=None, autocomplete=None, choices=None, aliases=None, nsfw=False, allowed_contexts=None, allowed_installs=None)`
...
| `nsfw` | `bool` | Mark command as age-gated |
| `allowed_contexts` | `discord.AppCommandContext \| None` | Where the command can be used |
| `allowed_installs` | `discord.AppInstallationType \| None` | Which install types can use it |
...
`bot.registry` — `InteractionRegistry` instance tracking every registered slash command.

`bot.registry.list_commands()` → `list[CommandEntry]` — all registered commands sorted by name. Subcommands under a `SlashGroup` appear as `"group_name cmd_name"`.

`bot.registry.commands_for_plugin(plugin_name: str)` → `list[CommandEntry]` — all commands registered by a specific plugin. Pass the class name as a string.

`CommandEntry` fields: `name: str`, `description: str`, `plugin: str | None`, `guild_id: int | None`, `is_alias: bool`.

```python
for entry in bot.registry.list_commands():
    print(entry.name, "→", entry.plugin or "Bot")

# Inspect one plugin's commands
for entry in bot.registry.commands_for_plugin("ModerationPlugin"):
    print(entry.name, entry.guild_id)
```
