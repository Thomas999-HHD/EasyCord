# EasyCord 
![Version](https://img.shields.io/badge/v-4.6.0-blue)
...
1. Install the latest release: `pip install "easycord @ git+https://github.com/rolling-codes/EasyCord.git@v4.6.0"`
...
**Bug fixes:**
- Fixed `asyncio.get_event_loop()` deprecation in all AI provider `query()` methods — now uses `asyncio.get_running_loop()` as required inside async functions (Python 3.10+)
- Bulk moderation helpers (`bulk_timeout`, `bulk_role_add`, `bulk_role_remove`) now log failures at `WARNING` level instead of swallowing them silently

**Breaking changes:**
- Removed dead `rate_limit` parameter from `@slash` decorator — it was stored on the method but never acted on; use the `cooldown` parameter instead

**Architecture:**
- `easycord/i18n.py` split into a proper sub-package (`easycord/i18n/`) — public API unchanged, all existing imports continue to work
