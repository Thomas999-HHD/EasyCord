...
- Direct access to raw `discord.Client` via `ctx.client` or `bot` (Bot IS discord.Client)
- Direct access to raw `discord.Interaction` via `ctx.raw_interaction`
- All configuration is plain JSON — no proprietary formats
- Plugins are plain Python — no runtime magic
- Drop to discord.py directly anytime, no penalties
...
# Drop down anytime (raw discord.py)
await bot.client.http.ban(user.id, ctx.guild.id, reason="spam")
