...
class GeminiProvider(AIProvider):
    ...
    def _init_client(self) -> None:
        if self._client is None:
            ...
            # Use genai.Client for instance-level auth instead of the
            # module-level genai.configure(), which mutates a global and
            # races when multiple GeminiProviders with different keys exist.
...
class TogetherAIProvider(AIProvider):
    ...
    def _init_client(self) -> None:
        if self._client is None:
            ...
            # Use Together(api_key=...) for instance-level auth instead of
            # setting together.api_key (module-level global) which races when
            # multiple TogetherAIProviders with different keys exist.
