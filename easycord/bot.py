...
from .kernel import EventBus, CapabilityRegistry
...
class Bot(_EventsMixin, _GuildMixin, _PluginsMixin, _CommandsMixin, discord.Client):
    ...
    Escape hatches (drop down to discord.py anytime):
    - ``bot`` is a ``discord.Client`` (use bot.client directly)
    - ``bot.tree`` for low-level slash command tree access
    - ``ctx.client`` for Discord API access from Context
