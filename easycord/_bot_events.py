...
class _EventsMixin:
    ...
    def dispatch(self, event: str, /, *args, **kwargs) -> None:
        ...
        # Emit to EventBus for v4.0 listeners (new style)
        # Convert discord.py event name to v4.0 event name
        # e.g., "ready" → "bot.ready", "member_join" → "member.join"
        event_v4 = event.replace("_", ".")
        if not event_v4.startswith("interaction"):
            # Non-interaction events use "bot.*" namespace
            event_v4 = f"bot.{event_v4}"

        bus_event = Event(event_v4, {"args": args, "kwargs": kwargs})
        asyncio.create_task(self.events.emit(bus_event))

        # Also call legacy v3 handlers for backwards compat
    ...
    def middleware(self, fn: MiddlewareFn) -> MiddlewareFn:
        """Alias for use() — register middleware (v4.0 style).
