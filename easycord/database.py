...
class MemoryDatabase(EasyCordDatabase):
    ...
    async def get_guild(self, guild_id: int) -> GuildRecord | None:
        async with self._lock:
            ...
            if data is None:
                return None
            return GuildRecord(guild_id=guild_id, data=copy.deepcopy(data))

    async def list_guilds(self) -> list[GuildRecord]:
        async with self._lock:
            return [GuildRecord(guild_id=guild_id, data=copy.deepcopy(data)) for guild_id, data in self._guilds.items()]

    async def get(self, guild_id: int, key: str, default: Any = None) -> Any:
        async with self._lock:
            value = self._guilds.get(guild_id, {}).get(key, default)
            # Return a deep copy so callers cannot mutate the in-memory store,
            # matching the SQLiteDatabase behaviour (which always deserialises
            # from JSON and therefore returns a fresh object every time).
            return copy.deepcopy(value)

    async def set(self, guild_id: int, key: str, value: Any) -> None:
        async with self._lock:
            self._guilds.setdefault(guild_id, {})[key] = copy.deepcopy(value)
