...
class ToolRegistry:
    ...
    def can_execute(self, ctx: Context, tool_name: str) -> tuple[bool, Optional[str]]:
        """Check if context can invoke tool."""
        ...
        if tool.allowed_roles:
            user_roles = {r.id for r in ctx.member.roles}
    ...
    async def execute(
        ...
        allowed, reason = self.can_execute(ctx, call.name)
        if not allowed:
    ...
    def list_available(self, ctx: Context) -> list[ToolDef]:
        """List tools executable by this context."""
        return [
            t
            for t in self._tools.values()
            if self.can_execute(ctx, t.name)[0]
        ]
