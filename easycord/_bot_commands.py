...
class _CommandsMixin:
    ...
    def _register_slash(
        ...
        *,
        ...
        # Register in our registry FIRST (cheap, raises on duplicate).
        # If the tree then rejects the command we roll back the registry entry
        # so neither source of truth holds a stale record.
        if parent is None:
            ...
            # Subcommands under a group must also be tracked so that
            # registry.list_commands() and commands_for_plugin() are complete,
            # and so unload can call unregister_command() on them.
            registry_name = f"{parent.name} {name}"
            try:
                self.registry.register_command(
                    registry_name,
                    description,
                ...
                # Duplicate — let the caller surface this via the parent.add_command
                # path; don't block registration of the discord.py command itself.
                logger.warning("Duplicate subcommand registry entry %r", registry_name)
