    ...
    def _build_slash_callback(
        ...
        *,
        ...
        async def callback(interaction: discord.Interaction, **kwargs) -> None:
            ...
            # Emit interaction.received event (pre-middleware, informational)
            ...
            asyncio.create_task(bot.events.emit(event))
            ...
            async def invoke() -> None:
                ...
                # Capability check (new v4.0 system)
                if capabilities:
                    ...
                    if not granted:
                        await ctx.respond(
                            ctx.t(
                                ...
                                default="You don't have the required capabilities.",
                            ),
                            ephemeral=True,
                        )
                        return
                ...
                try:
                    ...
                    error_event = Event("interaction.error", {
                        "ctx": ctx,
                        "exception": exc,
                        "command_name": ctx.command_name,
                    })
                    asyncio.create_task(bot.events.emit(error_event))
