    ...
    async def bulk_timeout(
        ...
        for user_id in user_ids:
            try:
                ...
                logger.warning("bulk_timeout: failed for user %d", user_id, exc_info=True)
    ...
    async def bulk_role_add(ctx: Context, user_ids: list[int], role_id: int) -> dict[int, bool]:
        ...
        for user_id in user_ids:
            try:
                ...
                logger.warning("bulk_role_add: failed for user %d", user_id, exc_info=True)
                results[user_id] = False
        return results
    ...
    async def bulk_role_remove(ctx: Context, user_ids: list[int], role_id: int) -> dict[int, bool]:
        ...
        for user_id in user_ids:
            try:
                ...
                logger.warning("bulk_role_remove: failed for user %d", user_id, exc_info=True)
                results[user_id] = False
        return results
