...
class _PluginsMixin:
    ...
    def _scan_methods(self, plugin: Plugin, *, parent=None) -> None:
        ...
        for _, method in inspect.getmembers(plugin, predicate=inspect.ismethod):
            ...
            if getattr(method, "_is_ai_tool", False):
                ...
                rl_tuple = getattr(method, "_ai_tool_rate_limit", None)
                rate_limit = RateLimit(*rl_tuple) if rl_tuple else None
                self.ai_tools[tool_name] = {
                    ...
                    "permissions": list(getattr(method, "_ai_tool_permissions", [])),
                }
        ...
        for _, method in inspect.getmembers(plugin, predicate=inspect.ismethod):
            if getattr(method, "_is_slash", False):
                ...
                for cmd_name in [method._slash_name] + list(getattr(method, "_slash_aliases", [])):
                    ...
                    self.registry.unregister_command(cmd_name)
            if getattr(method, "_is_event", False):
                try:
                ...
                if cleanup is not None:
                    try:
                        ...
                        logger.exception(
                            "Error in on_cleanup for %r in %s",
                            method.__name__,
            ...
            if getattr(method, "_is_ai_tool", False):
                self.ai_tools.pop(method._ai_tool_name, None)
        # Sweep any registry commands still attributed to this plugin (catches
        # subcommands registered under a group, whose registry key is namespaced
        # "group_name cmd_name" and wouldn't match the bare cmd_name above).
        plugin_name = type(plugin).__name__
        stale = [e.name for e in self.registry.commands_for_plugin(plugin_name)]
        for cmd_name in stale:
            self.registry.unregister_command(cmd_name)
    ...
    async def reload_plugin(self, name: str) -> None:
        """Reload a plugin by class name, re-registering all commands and handlers.

        Uses ``remove_plugin`` for full teardown (deregisters commands/events/tasks,
        calls ``on_unload``), then re-adds the same instance without the duplicate-check
        that ``add_plugin`` enforces. The same instance is reused so constructor
        arguments and non-command state are preserved.

        ``on_load`` is awaited directly (not scheduled via ``create_task``), so any
        exception it raises propagates to the caller. If ``on_load`` fails, the plugin
        is removed again to avoid a half-initialized state.
            ...
            if type(plugin).__name__ == name:
                ...
                if self.is_ready():
                    try:
                        ...
                        logger.exception(
                            "on_load failed during reload of %s — rolling back", name
                        )
                        # Don't call remove_plugin here — the plugin was already
                        # removed above and re-added to _plugins; calling remove_plugin
                        # again would recurse into a ValueError.  Manually undo the
                        # partial re-add instead.
                        self._plugins.remove(plugin)
                        raise
                return
