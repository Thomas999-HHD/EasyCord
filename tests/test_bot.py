...
def test_dispatch_calls_registered_handlers(bot):
    ...
    # v3.9: EventBus.emit + legacy handler = 2 create_task calls
...
def test_dispatch_skips_unregistered_events(bot):
    ...
    # v3.9: EventBus.emit is always called (even without legacy listeners)
    # EventBus has no matching listeners, but emit coroutine is still created_task'd
...
def test_dispatch_calls_multiple_handlers(bot):
    ...
    # v3.9: EventBus.emit + 2 handlers = 3 create_task calls
