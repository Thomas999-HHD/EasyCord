...
def test_detect_os_locale_returns_none_on_error():
    with patch("easycord.i18n._utils.stdlib_locale.getdefaultlocale", side_effect=ValueError):
        assert detect_os_locale() is None


def test_detect_os_locale_formats_correctly():
    with patch("easycord.i18n._utils.stdlib_locale.getdefaultlocale", return_value=("en", "US")):
        assert detect_os_locale() == "en-US"


def test_detect_os_locale_with_language_only():
    with patch("easycord.i18n._utils.stdlib_locale.getdefaultlocale", return_value=("fr", None)):
...
def test_auto_detect_locale_priority_system_over_default():
    with patch("easycord.i18n._utils.stdlib_locale.getdefaultlocale", return_value=("fr", "FR")):
        i18n = LocalizationManager(
            default_locale="en-US",
            auto_detect_system_locale=True,
            translations={"en-US": {}, "fr-FR": {}},
...
def test_auto_detect_locale_invalid_locale_silent_when_disabled(caplog):
    ...
    with patch("easycord.i18n._manager.logger") as mock_logger:
