...
## v4.6.0 - Stable Release

**Release Date:** 2026-05-04

### Highlights

- Fixed `asyncio.get_event_loop()` deprecation in all 9 AI provider `query()` methods
- Bulk moderation helpers now log failures at WARNING level
- Removed dead `rate_limit` parameter from `@slash` decorator (use `cooldown` instead)
- `easycord/i18n.py` refactored into `easycord/i18n/` sub-package (public API unchanged)
- Plugin command registry: slash commands tracked with name, description, plugin, and guild scope
- 691 tests passing, zero regressions

See full notes: [`RELEASE_v4.6.0.md`](RELEASE_v4.6.0.md).
...
- Documented localization fallback, Levels XP cooldown, invite tracking, middleware, plugin reload, context-menu, webhook, emoji, SQLite, and limiter fixes.
...
- `@slash` `rate_limit` param removed in v4.6.0 (was stored but never enforced; use `cooldown` instead)
